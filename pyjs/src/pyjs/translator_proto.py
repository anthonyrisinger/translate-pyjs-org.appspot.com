...
def translate(sources, output_file, module_name=None, **kw):
    ...
    if not module_name:
        module_name, extension = os.path.splitext(sources[0].filename)

    trees = []
    tree= None
    for src in sources:
        current_tree = compiler.parseFile(src.file)
        src.file.seek(0)
        flags = set()
        for l in src.file:
    ...
    src = sources[0].file.read()
    sources[0].file.seek(0)
    if list_imports:
    ...
    t = Translator(compiler,
                   module_name, sources[0].filename, src, tree, output, **kw)
